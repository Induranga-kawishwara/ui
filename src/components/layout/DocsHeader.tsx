...
export function DocsHeader({ onToggleSidebar }: { onToggleSidebar?: () => void }) {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 w-full border-b border-[#23283B] bg-[#080A12]/90 backdrop-blur-xl">
        <div className="flex h-16 items-center justify-between px-3 sm:px-6 lg:px-8 w-full max-w-7xl mx-auto">
          {/* Left: Mobile Docs Sidebar Trigger & Logo & Desktop Primary Nav */}
          <div className="flex items-center gap-2 sm:gap-6">
            {onToggleSidebar && (
              <button
                onClick={onToggleSidebar}
                className="md:hidden p-2 rounded-lg border border-[#23283B] bg-[#121625] text-[#94A3B8] hover:text-white hover:border-[#818CF8]/40 transition-all"
          ...
          {/* Right: Search bar & Links & Mobile Menu Trigger */}
          <div className="flex items-center gap-2 sm:gap-3">
            {/* Search Trigger Button - responsive width */}
            <button
              onClick={() => setIsSearchOpen(true)}
              className="flex items-center gap-2 p-2 sm:px-3 sm:py-1.5 rounded-lg border border-[#23283B] bg-[#0E1220]/80 text-[#94A3B8] hover:border-[#818CF8]/40 hover:text-white transition-all text-xs sm:w-52 md:w-56 justify-between group shadow-inner"
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg border border-[#23283B] bg-[#121625] text-xs font-medium text-[#CBD5E1] hover:text-white hover:border-[#818CF8]/40 hover:bg-[#818CF8]/10 transition-all group shrink-0"
            >
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 md:hidden flex justify-end">
          ...
          />
          <div className="relative z-50 w-72 max-w-[85vw] bg-[#0A0D17] border-l border-[#23283B] p-5 flex flex-col h-full overflow-y-auto shadow-2xl space-y-6">
            ...
            <div className="space-y-1.5">
              <span className="text-[10px] font-mono uppercase text-[#818CF8] font-bold tracking-wider px-3">
                Navigation
              </span>
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${
                        isActive
                          ? 'text-white bg-[#818CF8]/15 border border-[#818CF8]/30 font-semibold'
                          : 'text-[#94A3B8] hover:text-white hover:bg-white/5'
                      }`}
                    >
                      <span>{link.name}</span>
                      {isActive && <div className="w-1.5 h-1.5 rounded-full bg-[#818CF8] shadow-[0_0_6px_#818CF8]" />}
                    </Link>
                  );
                })}
              </div>
            ...
            <div className="pt-4 border-t border-[#23283B] space-y-3">
              <button
                onClick={() => {
                  setMobileMenuOpen(false);
                  setIsSearchOpen(true);
                }}
                className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl border border-[#23283B] bg-[#0E1220] text-xs text-[#94A3B8] hover:text-white hover:border-[#818CF8]/40 transition-all"
              >
                <div className="flex items-center gap-2">
                  <Search className="w-3.5 h-3.5 text-[#818CF8]" />
                  <span>Search documentation</span>
                </div>
                <span className="text-[10px] font-mono text-[#64748B]">⌘K</span>
              </button>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-[#23283B] bg-[#121625] text-xs font-medium text-[#CBD5E1] hover:text-white"
              >
                <GitHubIcon className="w-4 h-4 text-white" />
                <span>GitHub Repository</span>
                <span className="text-[#818CF8] font-mono text-[11px] ml-1">★ 2.4k</span>
              </a>
            </div>
          </div>
        </div>
